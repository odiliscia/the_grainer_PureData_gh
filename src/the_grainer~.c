...
/******************************************************************/
/*audio and envelope tables struct*/
typedef struct {
...
/******************************************************************/
/*list selection struct*/
typedef struct {
	int	size;	//list size
	int type;	//data selection type: RND, CYC, SHU
	int cont;	//internal counter
	t_float *data;
}LSEL;
/******************************************************************/					
typedef struct _the_grainer_tilde {
	...
	LSEL lsel[NLSELS];		/*list selection structures*/
	
	int   gap_count;      	/*gap samples counter*/
	int   gcountsiz;		/*how many grains we need to generate to output a bang*/
	int	  gcount;			/*grain counter*/
...
//just for pairs or single values
	if(what == PSES) {
		size	=x->lsel[what].size/2;
		add		=2;
	}
	else {
		size	=x->lsel[what].size;
		add		=1;	
	}


	switch (x->lsel[what].type) {
		case RND: {
				index=RAND_INT(size)*add;
				break;
		}
		case CYC: {
				index=x->lsel[what].cont;
				x->lsel[what].cont+=add;
				if(x->lsel[what].cont == x->lsel[what].size) x->lsel[what].cont=0;
				break;
		}
		case SHU: {
				if(x->lsel[what].cont == 0) { /*shuffle array content values every "size" times*/
					
						for(i=0; i < size; i++) {
							u=RAND_INT(size)*add;
							s=x->lsel[what].data[i*add];
							x->lsel[what].data[i*add]=x->lsel[what].data[u];
							x->lsel[what].data[u]=s;
							if(what==PSES) {
								s=x->lsel[what].data[i*add+1];
								x->lsel[what].data[i*add+1]=x->lsel[what].data[u+1];
								x->lsel[what].data[u+1]=s;
							}
						}	
				}
				index=x->lsel[what].cont;
...
if(strcmp(temp->s_name, "float") == 0) {
		...
		x->lsel[what].cont=0;	
			...
			x->lsel[what].size=argc-1;
			x->lsel[what].cont=0;
			if(strcmp(temp->s_name, "CYC") == 0)
				x->lsel[what].type=CYC;
			if(strcmp(temp->s_name, "SHU") == 0)
				x->lsel[what].type=SHU;	
		}

//if the list is larger than MAX_TABLES (24 elements), then do extra memory allocation, if needed
if(x->lsel[what].size > MAX_AUDIO_TABLES) {
	
	if(x->lsel[what].data)
		free(x->lsel[what].data);
		x->lsel[what].data=(t_float*)getbytes(sizeof(t_float)*x->lsel[what].size);
}
//load the data in the corresponding array		
for(i=0; i < x->lsel[what].size; ++i) {
		if(what == DEVS) {
				x->lsel[what].data[i]=(t_float)atom_getfloat(&argv[i]);	
			}
			else {
				x->lsel[what].data[i]=(t_float)fabs(atom_getfloat(&argv[i]));
			}
		if(what == AUDS) { /*must ensure that the tables requested were previously allocated*/
...
void the_grainer_tilde_any(t_the_grainer_tilde *x, t_symbol *string, t_int argc, t_atom *argv)
{
  ...
  if(strcmp(string->s_name, "atab") == 0) {
	...
	if(strcmp(string->s_name, "gcount") == 0) {
...
if(strcmp(string->s_name, "pitch_table") == 0) {
	...
	n_set_dval_sel(x, string, argc, argv, DEVS, "pitch");
	for(i=0; i < x->lsel[0].size; ++i) {
		x->lsel[0].data[i]=S2INCR(x->lsel[0].data[i]);
	}
return; 
}
/*gap values selection list*/
if(strcmp(string->s_name, "gap_table") == 0) {
	n_set_dval_sel(x, string, argc, argv, GAPS, "gap");
return; 
}
/*dur values selection list*/
if(strcmp(string->s_name, "dur_table") == 0) {
	n_set_dval_sel(x, string, argc, argv, DURS, "duration");
	x->lsel[PSES].size=0;
return; 
}
/*file access time values selection list*/
if(strcmp(string->s_name, "ptr_table") == 0) {
	n_set_dval_sel(x, string, argc, argv, PSTS, "audio table access times");
	x->lsel[PSES].size=0;
return; 
}
...
*/
	n_set_dval_sel(x, string, argc, argv, PSES, "audio table access and size times");
	/*set durs and acces times list to 0*/
	x->lsel[DURS].size=x->lsel[PSTS].size=0;
return; 
}
/*audio table numbers selection list*/
  if(strcmp(string->s_name, "gtab_table") == 0) {
	n_set_dval_sel(x, string, argc, argv, AUDS, "audio table");
  return;
  }
/*********************************************************************************/
...
x->out_bang = outlet_new(&x->x_obj, &s_bang);		/*for bang each time n grains are generated*/
...
if(which==GTAB) {							/*audio oscillator*/
	...
	if(!x->lsel[AUDS].size) {		
		...
		oscil->nt=(int)get_float(x,AUDS);	/*random selection*/			
	}
...
if(x->nch != MONO) {
	...
	we use here the FuMa channel ordering with SN3D normalization factors for encoding
...
GRAIN *grain_setup(t_the_grainer_tilde *x)
{
	...
	this may produce a serious crash.*/
	...
	if(!x->lsel[PSES].size) {
		...
		faux=x->lsel[DURS].size? set_birand_val(get_float(x,DURS),x->sgrnd) : set_birand_val(x->sgr, x->sgrnd); 	
		if(faux == 0.) faux=MIN_DUR;
		if(faux <  0.) faux*=-1.;
		g->size=floor(faux*(t_float)x->sr);
		/*compute phase offset for the grain table lookup from stgr*/
		if(x->lsel[PSTS].size) {
			from=set_birand_val(get_float(x,PSTS),x->stgrnd);
		...
		aux= (int)get_float(x, PSES);
		faux	=	set_birand_val(x->lsel[PSES].data[aux+1],x->sgrnd);
		if(faux == 0.) 	faux=MIN_DUR;
		if(faux <  0.) 	faux*=-1.;
		g->size=floor(faux*(t_float)x->sr);
		from	=      set_birand_val(x->lsel[PSES].data[aux],x->stgrnd);
	...
	if(x->lsel[0].size) {
		g->incr=x->fgr*(set_birand_val(get_float(x,DEVS), x->fgrnd));
	}
	else {
	...
	/*bang each gcountsiz grains, if requested*/
	if(x->gcountsiz) {
		if(!--x->gcount) {
			outlet_bang(x->out_bang);
			x->gcount=x->gcountsiz;	
		}	
	}
	return(g);
}
...
aux= x->lsel[GAPS].size? set_birand_val(get_float(x,GAPS), x->gagrnd) : set_birand_val(x->gagr, x->gagrnd);
...
for(i=0; i < NLSELS; ++i) { 
	...
	x->lsel[i].type=0;
	x->lsel[i].cont=0;
//the lists for data values selection are initially of lenght MAX_TABLES (24 elements)
//we'll further enlarge them if it is needed
	x->lsel[i].data=(t_float*)getbytes(MAX_TABLES*sizeof(t_float)); 
}
...
/*set the grain counter to zero*/
x->gcount=x->gcountsiz=0;
